...
/* Calculates the total no of bytes sent over a duration and calculates the throughput */
	...
	double speed = (((BytesReceivedIPV4[str] * 8.0) / 1024)/(time-startTime));
	*stream->GetStream() << time-startTime << "\t" <<  speed<< std::endl;
	if(maximum[str] < speed)	//storing the maximum throughput achieved
		maximum[str] = speed;
...
/* Calculates the no of useful bytes sent from no of packets dropped till that point and total bytes sent and calculates the goodput */
	...
	double speed = (((TotalBytes[addr] * 8.0) / 1024)/(time-startTime));
	*stream->GetStream() << time-startTime << "\t" <<  speed << std::endl;
}
...
/* General function to take the name of TCP protocol as function parameter and set up the corresponding TCP socket	*/
	...
	app->Setup(TcpSocket, sinkAddress, packetSize, numPackets, DataRate(dataRate)); //creating the application process
...
void part_B() {
	...
	p2pHR.SetQueue("ns3::DropTailQueue<Packet>", "MaxSize", QueueSizeValue(QueueSize("163p")));
	...
	********************************************************************/
	...
	Ptr<OutputStreamWrapper> stream1CWND = asciiTraceHelper.CreateFileStream("application_1_h1_h4_b.cwnd");
	Ptr<OutputStreamWrapper> stream1PD = asciiTraceHelper.CreateFileStream("application_1_h1_h4_b.congestion_loss");
	Ptr<OutputStreamWrapper> stream1TP = asciiTraceHelper.CreateFileStream("application_1_h1_h4_b.tp");
	Ptr<OutputStreamWrapper> stream1GP = asciiTraceHelper.CreateFileStream("application_1_h1_h4_b.gp");
	Ptr<Socket> ns3TcpSocket1 = flow(InetSocketAddress(receiverIFCs.GetAddress(0), port), port, "TcpHybla", senders.Get(0), receivers.Get(0), oneFlowStart, oneFlowStart+durationGap, packetSize, numPackets, transferSpeed, oneFlowStart, oneFlowStart+durationGap);

	ns3TcpSocket1->TraceConnectWithoutContext("CongestionWindow", MakeBoundCallback (&CwndUpdater, stream1CWND, 0));
	...
	Config::Connect(sink, MakeBoundCallback(&goodput, stream1GP, 0));

	std::string sink_ = "/NodeList/5/$ns3::Ipv4L3Protocol/Rx";
	Config::Connect(sink_, MakeBoundCallback(&Throughput, stream1TP, 0));


	
	//TCP Westwood from H2 to H5
	Ptr<OutputStreamWrapper> stream2CWND = asciiTraceHelper.CreateFileStream("application_1_h2_h5_b.cwnd");
	Ptr<OutputStreamWrapper> stream2PD = asciiTraceHelper.CreateFileStream("application_1_h2_h5_b.congestion_loss");
	Ptr<OutputStreamWrapper> stream2TP = asciiTraceHelper.CreateFileStream("application_1_h2_h5_b.tp");
	Ptr<OutputStreamWrapper> stream2GP = asciiTraceHelper.CreateFileStream("application_1_h2_h5_b.gp");
	Ptr<Socket> ns3TcpSocket2 = flow(InetSocketAddress(receiverIFCs.GetAddress(1), port), port, "TcpWestwood", senders.Get(1), receivers.Get(1), otherFlowStart, otherFlowStart+durationGap, packetSize, numPackets, transferSpeed, otherFlowStart, otherFlowStart+durationGap);
	ns3TcpSocket2->TraceConnectWithoutContext("CongestionWindow", MakeBoundCallback (&CwndUpdater, stream2CWND, 0));
	...
	Config::Connect(sink, MakeBoundCallback(&goodput, stream2GP, 0));
	sink_ = "/NodeList/6/$ns3::Ipv4L3Protocol/Rx";
	Config::Connect(sink_, MakeBoundCallback(&Throughput, stream2TP, 0));

	//TCP Yeah from H3 to H6
	Ptr<OutputStreamWrapper> stream3CWND = asciiTraceHelper.CreateFileStream("application_1_h3_h6_b.cwnd");
	Ptr<OutputStreamWrapper> stream3PD = asciiTraceHelper.CreateFileStream("application_1_h3_h6_b.congestion_loss");
	Ptr<OutputStreamWrapper> stream3TP = asciiTraceHelper.CreateFileStream("application_1_h3_h6_b.tp");
	Ptr<OutputStreamWrapper> stream3GP = asciiTraceHelper.CreateFileStream("application_1_h3_h6_b.gp");
	Ptr<Socket> ns3TcpSocket3 = flow(InetSocketAddress(receiverIFCs.GetAddress(2), port), port, "TcpYeah", senders.Get(2), receivers.Get(2), otherFlowStart, otherFlowStart+durationGap, packetSize, numPackets, transferSpeed, otherFlowStart, otherFlowStart+durationGap);
	ns3TcpSocket3->TraceConnectWithoutContext("CongestionWindow", MakeBoundCallback (&CwndUpdater, stream3CWND, 0));
	...
	Config::Connect(sink, MakeBoundCallback(&goodput, stream3GP, 0));
	sink_ = "/NodeList/7/$ns3::Ipv4L3Protocol/Rx";
	Config::Connect(sink_, MakeBoundCallback(&Throughput, stream3TP, 0));
